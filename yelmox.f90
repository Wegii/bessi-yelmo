program yelmox
    ...
    call marshelf_init(mshlf1,path_par,"marine_shelf",yelmo1%grd%nx,yelmo1%grd%ny,domain, &
                       yelmo1%par%grid_name,yelmo1%bnd%regions,yelmo1%bnd%basins)
    ...
    if (yelmo1%par%use_restart) then 
        ...
        if (trim(yelmo1%par%domain) .eq. "Laurentide" .or. trim(yelmo1%par%domain) .eq. "North") then 
            ...
            where (yelmo1%bnd%regions .eq. 1.1 .and. yelmo1%grd%lat .gt. 50.0 .and. &
                        yelmo1%bnd%z_bed .gt. 0.0 .and. yelmo1%bnd%smb .lt. 0.0 ) yelmo1%bnd%smb = .5 
    ...
    do n = 1, ceiling((time_end-time_init)/dtt)
        ...
        if (calc_transient_climate) then 
            ...
            !mmr - this gives problems with restart when dtt is small if (mod(time,2.0)==0) then            
            if (mod(time,dtt)==0) then 
                ! Update snapclim
